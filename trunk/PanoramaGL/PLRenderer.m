...
-(void)performClick:(NSArray *)params;
...
-(id)initWithView:(PLViewBase *)aView scene:(NSObject<PLIScene> *)aScene;
{
	if(self = [self init])
	{
		isValid = FALSE;
...
{	
	@synchronized(self)
	{
		if(backingWidth != view.bounds.size.width || backingHeight != view.bounds.size.height)
		{
			isValid = NO;
		...
		if([element isKindOfClass:[PLHotspot class]])
		{
			...
			if([PLIntersection checkLineBoxWithRay:rayVectors point1:points[0] point2:points[1] point3:points[2] point4:points[3] hitPoint:&hitPoint])
			{
				...
				if(isMoving)
				{
					if(hotspot.touchStatus == PLHotspotTouchStatusOut)
					{
						...
						if(delegate && [delegate respondsToSelector:@selector(view:didOverHotspot:screenPoint:scene3DPoint:)])
							[delegate view:view didOverHotspot:hotspot screenPoint:screenPoint scene3DPoint:hitPoint.position];
					}
					else
				...
				else
				{
					...
					NSArray *params = [NSArray arrayWithObjects:hotspot,
									   [NSNumber numberWithFloat:screenPoint.x],
									   [NSNumber numberWithFloat:screenPoint.y],
									   [NSNumber numberWithFloat:hitPoint.x],
									   [NSNumber numberWithFloat:hitPoint.y],
									   [NSNumber numberWithFloat:hitPoint.z],
									   nil];
					[self performSelector:@selector(performClick:) withObject:params afterDelay:0.1];
					break;
				}
				hits++;
			}
			else
			{
				if(hotspot.touchStatus != PLHotspotTouchStatusOut)
				{
					if(delegate && [delegate respondsToSelector:@selector(view:didOutHotspot:screenPoint:scene3DPoint:)])
						[delegate view:view didOutHotspot:hotspot screenPoint:screenPoint scene3DPoint:hitPoint.position];
...
{
	NSObject<PLViewDelegate> *delegate = view.delegate;
	PLHotspot *hotspot = (PLHotspot *)[params objectAtIndex:0];
	...
	if(delegate && [delegate respondsToSelector:@selector(view:didClickHotspot:screenPoint:scene3DPoint:)])
	{
		[delegate view:view
			...
			screenPoint:CGPointMake([((NSNumber *)[params objectAtIndex:1]) floatValue], [((NSNumber *)[params objectAtIndex:2]) floatValue])
			scene3DPoint:PLPositionMake([((NSNumber *)[params objectAtIndex:3]) floatValue], [((NSNumber *)[params objectAtIndex:4]) floatValue], [((NSNumber *)[params objectAtIndex:5]) floatValue])
		];
	}
	view.isBlocked = NO;
}
