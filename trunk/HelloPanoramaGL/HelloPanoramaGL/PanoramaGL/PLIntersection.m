...
+(BOOL)checkLineTriangleWithRay:(PLVector3 **)ray firstVertex:(PLVector3 *)firstVertex secondVertex:(PLVector3 *)secondVertex thirdVertex:(PLVector3 *)thirdVertex hitPoint:(PLVector3 **)hitPoint
{
	...
	*hitPoint = intersect;
	...
	/*PLVector3 *aux, *intersect, *normal;
	
	//Calculate triangle normal
	aux = [secondVertex sub:firstVertex];
	normal = [aux crossProductAndRelease:[thirdVertex sub:firstVertex]];
	[normal normalize];
	
	//Find distance from ray to the plane defined by the triangle
	aux = [ray[0] sub:firstVertex];
	float distance1 = [aux dot:normal];
	
	aux = [ray[1] sub:firstVertex];
	float distance2 = [aux dot:normal];
	...
	//Find point on the ray that intersects with the plane
	aux = [ray[1] sub:ray[0]];
	intersect = [ray[0] addAndRelease:[aux multf:-distance1/(distance2-distance1)]];
	[aux release];

	//Find if the interesection point lies inside the triangle by testing it against all edges
	PLVector3 * test;
	test = [normal crossProductAndRelease:[secondVertex sub:firstVertex]];
	if([test dotAndRelease:[intersect sub:firstVertex]] < 0.0f)
	{
		[test release];
		[intersect release];
		[normal release];
	...
	[test release];
	test = [normal crossProductAndRelease:[thirdVertex sub:secondVertex]];
	if([test dotAndRelease:[intersect sub:secondVertex]] < 0.0f)
	{
		[test release];
		[intersect release];
		[normal release];
	...
	[test release];
	test = [normal crossProductAndRelease:[firstVertex sub:thirdVertex]];
	if([test dotAndRelease:[intersect sub:firstVertex]] < 0.0f)
	{
		[test release];
		[intersect release];
